...
`fromEnv()` provides a convenience configuration path based on compile-time `--dart-define` values. It can select the cloud, local, or custom backend provider.
...
1. scanning mixed markdown/text for the first valid JSON object;
2. decoding known fields into the requested model;
3. retaining raw markdown/JSON data; and
4. returning safe empty structured fields when structured JSON cannot be recovered.

This is useful because LLM/VLM responses may include markdown fences or explanatory text around JSON.
...
Preview
      |
      v
TyphoonOCR.extract<ThaiIdCard>()
      |
      v
Structured result + checksum
```
...
- **client orchestration** — correct definition, prompt, mode, and typed result;
- **parser/model behavior** — mixed markdown JSON, fallback behavior, rich fields, MRZ preservation, Thai ID checksum;
- **provider contract** — HTTP URL, authorization, model/image payload, successful response extraction, and typed HTTP failures.
